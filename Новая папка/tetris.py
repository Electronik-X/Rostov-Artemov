...
class Piece(object):
    def __init__(self, x, y, shape):  # создание фигуры
        self.x = x
        self.y = y
        self.shape = shape
...
def draw_window(surface, grid, score=0, last_score = 0):
    ...
    label = font.render('Наилучший счет: ' + last_score, 1, (255,255,255))
    sx = top_left_x - 200
    sy = top_left_y + 200
...
def main(win):  # *
    ...
    locked_positions = {}
    grid = create_board(locked_positions)
    change_piece = False
    run = True
...
def main_menu(win):  # *
    ...
    while run:
        win.fill((0, 0, 0))
        draw_text_middle(win, 'Нажмите любую клавишу, чтобы начать', 30, (255, 255, 255))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
